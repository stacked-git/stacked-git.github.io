...
When using the --expose option, the format of the commit message is determinded
by the 'stgit.pick.expose-format' configuration option. This option is a format
string as may be supplied to the --pretty option of linkgit:git-show[1]. The
default is "format:%B%n(imported from commit %H)", which appends the commit
